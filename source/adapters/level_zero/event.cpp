...
ur_queue_handle_legacy_t_::enqueueEventsWaitWithBarrier( ///< [in] handle of the
      ...
      if (EventWaitList.Length) {
        if (CmdList->second.IsInOrderList) {
          for (unsigned i = EventWaitList.Length; i-- > 0;) {
            // If the event is a multidevice event, then given driver in order lists, we cannot include this into the wait event list due to driver limitations.
  ...
  if (CmdLists.size() > 1) {
    ...
    _ur_ze_event_list_t waitlist;
    UR_CALL(
        insertBarrierIntoCmdList(CmdLists[0], waitlist, *Event, IsInternal));
  }
...
ur_result_t _ur_ze_event_list_t::createAndRetainUrZeEventList(
  ...
  try {
    ...
    if (EventListLength > 0) {
	  ...
	  // Append a Barrier to wait on the original event while signalling the new multi device event.
	  ZE2UR_CALL(zeCommandListAppendBarrier, (ZeCommandList, MultiDeviceZeEvent, 1u, &EventList[I]->ZeEvent));
